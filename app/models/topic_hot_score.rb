...
class TopicHotScore < ActiveRecord::Base
  ...
  def self.hottest_topic_ids
    hot_topic_ids_cache["ids"].to_a
  end

  def self.recreate_hottest_topic_ids_cache
    hot_topic_ids_cache.clear

    hot_topic_ids_cache.defer_get_set("ids") do
      DB.query_single("SELECT topic_id FROM topic_hot_scores ORDER BY score DESC LIMIT 100")
    end
  end
